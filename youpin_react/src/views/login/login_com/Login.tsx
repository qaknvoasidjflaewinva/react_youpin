...
function Login() {
    ...
    function loginBtn(e: any) {
        e.preventDefault();
        if (no.length == 0 && pwd.length == 0) {
    ...
    return (
        <form onSubmit={loginBtn}>
            ...
            <div className="mi-input mi-text-field__input">
                <input style={input} onClick={hasNo} type="text" name="account" className="mi-input__input" value={no} onChange={(e: any) => { setNo(e.target.value) }} />
                <label className="mi-floating-label" style={labStyle1}>邮箱/手机号码/小米ID</label>
            </div>
            <span style={{ marginTop: '10px', display: accState[0] == true ? 'none' : 'block', color: '#F0605D' }}>请输入账号</span>
            ...
            <span style={{ marginTop: '10px', display: accState[1] == true ? 'none' : 'block', color: '#F0605D' }}>请输入密码</span>
            ...
            <button onClick={loginBtn} className='login-btn' style={{ opacity: isChecked ? '1' : '.4' }} disabled={!isChecked}>登录</button>
            <div className='tab-actions'>
                <div className='tab'>忘记密码</div>
                <div className='tab1'>手机号登录</div>
            </div>
